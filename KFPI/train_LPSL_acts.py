...
from dataset import KFPIDataset,KFPIDatasetAL
from acts import Acts

parser = argparse.ArgumentParser(description="KFPI LPSL ACTS Running")
...
parser.add_argument('--E1', default=16, type=int)
...
n_classes = 70 # total class number
HLevel = [1,2,3] # Level 3
lambda1=args._lambda 
lambda2=args.eta 
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

train_loss_file = "/home/wyx/vscode_projects/hier/models/KFPI/acts_train_loss_lpsl_acts_delta{}T{}.csv".format(args.delta,args.T1)
val_loss_file = "/home/wyx/vscode_projects/hier/models/KFPI/acts_val_loss_lpsl_acts_delta{}T{}.csv".format(args.delta,args.T1)
test_acc_file = "/home/wyx/vscode_projects/hier/models/KFPI/acts_test_acc_lpsl_acts_delta{}T{}.csv".format(args.delta,args.T1)
best_model_path = "/home/wyx/vscode_projects/hier/models/KFPI/acts_best_model_lpsl_delta{}T{}.pth".format(args.delta,args.T1)
acts_out_path = "/home/wyx/vscode_projects/hier/models/KFPI/acts_Q_delta{}T{}.csv".format(args.delta,args.T1)
...
# 读取数据
dataset_L,dataset_U = KFPIDatasetAL.createLURandomly(HLevel=HLevel,image_size=32)
dataset_test = KFPIDataset(train=False, HLevel=HLevel, image_size=32)
...
# loss 类型
criterion = LPSL(KFPIDataset.CLASS_HIERARCHY,lambda1,lambda2)
...
def val(epoch):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            loss = criterion.loss(output, target)
            sum_loss += loss.data.item()
            _, pred1 = torch.max(output.data[..., :4], dim=1)
            _, real1 = torch.max(target.data[..., :4], dim=1)
            _, pred2 = torch.max(output.data[..., 4:25], dim=1)
            _, real2 = torch.max(target.data[..., 4:25], dim=1)
            _, pred3 = torch.max(output.data[..., 25:], dim=1)
            _, real3 = torch.max(target.data[..., 25:], dim=1)
