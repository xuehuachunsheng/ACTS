...
parser = argparse.ArgumentParser(description="FashionMNIST LPSL Running")
...
n_classes = 18 # total class number
HLevel = [1,2,3] # Level 3
lambda1=args._lambda 
lambda2=args.eta 
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

train_loss_file = "/home/wyx/vscode_projects/hier/models/FashionMNIST/acts_train_loss_lpsl_acts_delta{}T{}.csv".format(args.delta,args.T1)
val_loss_file = "/home/wyx/vscode_projects/hier/models/FashionMNIST/acts_val_loss_lpsl_acts_delta{}T{}.csv".format(args.delta,args.T1)
test_acc_file = "/home/wyx/vscode_projects/hier/models/FashionMNIST/acts_test_acc_lpsl_acts_delta{}T{}.csv".format(args.delta,args.T1)
best_model_path = "/home/wyx/vscode_projects/hier/models/FashionMNIST/acts_best_model_lpsl_delta{}T{}.pth".format(args.delta,args.T1)
acts_out_path = "/home/wyx/vscode_projects/hier/models/FashionMNIST/acts_Q_delta{}T{}.csv".format(args.delta,args.T1)
...
# 读取数据
dataset_L,dataset_U = FashionMNISTDatasetAL.createLURandomly(image_size=32)
dataset_test = FashionMNISTDataset(train=False, HLevel=HLevel, image_size=32)
...
# loss 类型
criterion = LPSL(FashionMNISTDataset.CLASS_HIERARCHY,lambda1,lambda2)

# Original version, the 1st convolution layer is 7x7
...
_acts_ = Acts(dataset_L, dataset_U, 
              ...
              args.q_budget, 
              DEVICE, 
              FashionMNISTDataset.CLASS_HIERARCHY, 
              delta=args.delta, 
              m=E1-1, 
              T1=args.T1, 
              Tm=0.001,
...
def val(epoch):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            _, pred2 = torch.max(output.data[..., 2:8], dim=1)
            _, real2 = torch.max(target.data[..., 2:8], dim=1)
            _, pred3 = torch.max(output.data[..., 8:], dim=1)
            _, real3 = torch.max(target.data[..., 8:], dim=1)
